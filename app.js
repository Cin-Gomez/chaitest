...
//add a record- must have name and age- cannot be neg. number
...
app.get("/api/v1/people/:id", (req, res) =>{
  ...
  if (
    isNaN(index) || !Number.isInteger(index) || index < 0 || index >= people.length
    ) {
      res.status(404).json({message: "A person record was not found"});
      return;
    }
